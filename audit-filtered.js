...
class AuditTrailFiltered extends BaseReport {
    constructor() {
        super();
    }
    ...
    async formatHtml(formattedData) {
        return format('auditFiltered',formattedData);
    }

    async formatXml(contents) {
        return array2xml('auditFiltered',contents);
    }

    async formatArray(data,notes,filter="Update") {
        var _data = {
            title: `Audit Trail (Filtered: Only by ${filter})`,
    ...
    retrieveReportData(startDate,endDate,callback,filter="Update") {
        callback(this.retrieveReportDataSync(startDate,endDate,filter));
    }

    async retrieveReportDataSync(startDate, endDate, filter="Update") {
        ...
        const sql = `SELECT l.txdate, l.account_code, l.account_name, l.debit, l.credit, l.balance, 
                    ...
                    WHERE l.txdate BETWEEN "${startDate}" AND "${endDate}" AND a.action = "${filter}"
...
module.exports = AuditTrailFiltered;