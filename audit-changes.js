...
class AuditTrailChanges extends BaseReport {
    constructor() {
        super();
    }
    ...
    async formatHtml(formattedData) {
        return format('auditChanges',formattedData);
    }

    async formatXml(contents) {
        return array2xml('auditChanges',contents);
    }
    ...
    async formatArray(data,notes) {
        ...
        data.forEach(function(transaction){
            transactions.push({
                ...
                oldAmount: transaction.old_balance,
                newAmount: transaction.new_balance
            })
        });
    ...
    async retrieveReportDataSync(startDate, endDate) {
        ...
        const sql = `SELECT l.txdate, l.account_code, l.account_name, l.debit, l.credit, l.balance, 
                    ...
                    WHERE l.txdate BETWEEN "${startDate}" AND "${endDate}" AND NOT a.old_balance = a.new_balance
...
module.exports = AuditTrailChanges;